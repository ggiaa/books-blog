    ...
    <div class="row col-lg-7 mb-5">
        <form action="/dashboard/books/{{ $book->slug }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" value="{{ old('title',$book->slug)  }}">
                @error('slug')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
                ...
                <select id="publisher_id" class="form-select @error('publisher_id') is-invalid @enderror" name="publisher_id">
                    ...
                    @foreach($publishers as $publisher)
                        @if (old('publisher_id', $book->publisher_id) == $publisher->id)
                            <option value="{{ $publisher->id }}" selected>{{ $publisher->company_name }}</option>                              
                        @else
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('total_pages') is-invalid @enderror" id="total_pages" name="total_pages" value="{{ old('total_pages',$book->total_pages)  }}">
                @error('total_pages')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control @error('publish_year') is-invalid @enderror" id="publish_year" name="publish_year" value="{{ old('publish_year',$book->publish_year)  }}">
                @error('publish_year')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
